...
const fetchQuizzes = async (token, catId) => {
  try {
    ...
    let quizzes = null;
    if (catId === null) {
      const { data } = await axios.get("/api/quizzes/all", config);
      quizzes = data;
    } else {
      const { data } = await axios.get(`/api/quizzes/id/${catId}`, config);
      quizzes = data;
    }
...
const addQuiz = async (quiz, token) => {
  try {
    ...
    const { data } = await axios.post("/api/quizzes/create-quiz", quiz, config);
